//Copyright 2019, Bradley Peterson, Weber State University, All rights reserved. (9/18)
#include <sstream>
#include <map>
#include <chrono>
#include <iostream>
...
BaseDoublyLinkedList<T>::~BaseDoublyLinkedList() {
	if (front) {
		// Chain reactions are bad, they can fill up the stack and cause a stack overflow.  
		// Instead avoid a chain reaction by keeping a reference to a node.
		while (front->forward) {
			front = front->forward;
			front->backward = nullptr;
		}
	}
}

template <typename T>
void BaseDoublyLinkedList<T>::insertFront(const T& item) {
...
template <typename T>
void BaseDoublyLinkedList<T>::insertBack(const T& item) {
	Node<T>* temp = new Node<T>();
...
T DoublyLinkedList<T>::get(const unsigned int index) const
{
	...
	if (currentNode == nullptr) {
		cout << "you ran out of nodes" << endl;
		throw 1;
	}
	else if (index == 0) {
...
void DoublyLinkedList<T>::insert(const unsigned int index, const T & value)
{
	...
	while (tempIn < index && currentNode != nullptr) {//keep moving forward in list if we havent hit our index

		currentNode = currentNode->forward;
		tempIn++;
	}
 if (this->front == this->back && currentNode != nullptr) { //if we have exactly one node

			temp->backward = currentNode;
			currentNode->forward = temp;
			this->back = temp; 

		}
	else if (currentNode == this->front && currentNode != nullptr) {//if we are adding to the first node
	...
	else if (currentNode == this -> back && currentNode != nullptr) {//if we are adding to the last node

		temp->backward = currentNode;
		currentNode->forward = temp; 
		this->back = temp; 
	}
	else if (!this->front) {// list is empty 
		this->front = temp; 
		this->back = temp; 
	}

	else {
		if (currentNode != nullptr) {//generic scenerio
			temp->backward = currentNode;
			temp->forward = currentNode->forward;
			currentNode->forward->backward = temp;
			currentNode->forward = temp;
		}
	}
...
//	Node<T> * temp = nullptr;
	...
	while (tempIn < index && currentNode != nullptr ) {
		//keep moving forward in list if we havent hit our index
	...
	//if node deleted is front 
		 else  if (currentNode == this->front && currentNode != nullptr) {
		Node<T> * temp{ currentNode->forward }; 
		delete currentNode; 
		this->front = temp;
		if (this->front) {
			this->front->backward = nullptr;
		}
		else {//one node 
			this->back = nullptr;
		}
	}
	else if (currentNode == this->back && currentNode != nullptr) { //if we are sayin bye to the last node
	
		this->back = currentNode->backward; 
		delete currentNode; 
		this->back->forward = nullptr; 
	}


	else {//generic middle scenerio
		if (currentNode != nullptr) {
			Node<T> * temp = nullptr;
...
void DoublyLinkedList<T>::removeAllInstances(const T & value) {
	...
	while (currentNode) {//if currentNode exist do these things: 

		if ( currentNode->data == value) {//if we reach a point where currnode=value
			

			 if (this->front == this->back) {//one node
				 delete currentNode; 
				 currentNode = nullptr; 
				 this->front = nullptr;
				 this->back = nullptr; 
			...
			else if (currentNode == this->front) {//fix logic
				currentNode = currentNode->forward;
				delete this->front;
				...
				Node<T>*node2delete = nullptr; //make a pointer so i can delete currnode
				node2delete = currentNode;
				currentNode = currentNode->forward;
				node2delete->forward->backward = node2delete->backward;
				node2delete->backward->forward = node2delete->forward;
				delete node2delete;
			}
